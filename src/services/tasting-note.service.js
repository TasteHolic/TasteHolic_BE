import { responseFromUser, toUserTastingNoteDTO, toExpertTastingNoteDTO } from "../dtos/tasting-note.dto.js";
import {
  searchDrinksInDB,
  addTastingNote,
  getTastingNote,
...
export const userTastingNoteDetail = async (type, noteId) => {
  ...
  export const tastingNoteDetail = async (type, id) => {
    // 사용자의 테이스팅 노트 조회
    const usertastingnote = await getUserTastingNote(type, id);
    const userTastingNote = toUserTastingNoteDTO(type,usertastingnote);
  
    // 전문가 테이스팅 노트 조회
    const noteid = type === "cocktail" ? userTastingNote.cocktailId : userTastingNote.alcoholId;
    const experttastingnote = await getExpertTastingNote(type, noteid);
