...
export const getFilteredRecipesFromDB = async (filter, cursor, limit) => {
  try {
    ...
    if (filter === "zero") {
      query = `
        SELECT id, nameKor, ingredientsEng, likes, views, imageUrl, glass, 'cocktail' as type
        FROM Cocktails
        WHERE abv = 0
      `;
    } else {
    ...
    query += `
      UNION ALL
      SELECT id, name, ingredients, likes, views, imageUrl, glassType, 'user' as type
      FROM UserRecipes
    `;
...
export const getFavRecipesFromDB = async (userId, cursor, limit) => {
  try {
    let values = [];
    let query = `
      SELECT ur.id, ur.name, ur.ingredients, ur.likes, ur.views, ur.imageUrl, ur.glassType,'user' as type
    ...
    query += `
      UNION ALL
      SELECT c.id, c.nameKor AS name, c.ingredientsEng AS ingredients, c.likes, c.views, c.imageUrl, c.glass, 'cocktail' as type
